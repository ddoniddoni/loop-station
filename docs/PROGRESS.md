...
- 프로젝트 위치: `/Users/ddoni/dev/loop-station`, 현재 브랜치 `feature/phase-1-input-channels`. Tap Tempo·녹음 길이 통합 후 develop `e569efc`에서 시작함. 이전 파일 백업·Git stash와 다른 기능 브랜치는 보존함.
...
- 별도 키보드 구현: `feature/phase-2-keyboard-controls` / `40dafae`에 구현·푸시되어 있으며 develop과 이번 브랜치에는 미통합. 단위 20개·E2E 5개는 미실행이며 해당 브랜치의 기록을 따름.
- 이번 입력 채널: 기본 모노 합산·왼쪽/CH1·오른쪽/CH2 선택, 실제 채널 수 기반 제한, 캡처 중 잠금과 모니터·미터 초기화를 구현함. 단위 16개·production PCM E2E 1개는 작성만 했으며 미실행.
- 다음 작업: AUTO 모니터링. 테스트 재개 요청 후 Phase 0 오디오 수명·production Worklet·마이크 권한/장치 해제부터 확인하고 입력 채널·Tap Tempo·PCM·믹서·저장 및 별도 키보드·카운트인 브랜치를 검증함.
...
| 1 | 한 트랙 녹음과 공통 시계 | 부분 구현 | 같은 Worklet 시계의 1/2/4/8마디 PCM 캡처·자동 반복·정지/재시작과 Tap Tempo, 모노 합산·CH1/CH2 선택 구현. 카운트인은 별도 기능 브랜치에 구현·미통합. AUTO 모니터링·스테레오 녹음·다른 퀀타이즈 설정과 실제 검증은 남음 |
...
아래 표는 Tap Tempo·녹음 길이 선택·v5 저장과 이번 입력 채널 코드를 기준으로 한다. 키보드·카운트인은 별도 브랜치에 구현·미통합 상태이며 해당 기록을 함께 확인한다.
...
| IN-01 | 입력 장치와 채널 선택 | 1 | 부분 구현 | 장치 목록·전환, 실제 채널 수 기반 모노 합산·CH1/CH2 선택과 공통 녹음/미터/모니터 라우팅 구현. 녹음 준비·대기·캡처 중 장치·채널 변경과 연결 재시도 차단. 스테레오 녹음·다채널 전체 선택과 실제 장치/브라우저 검증은 남음 |
| IN-02 | 입력 게인과 모니터링 | 1 | 부분 구현 | 선택한 모노 입력 버스, −24~+24 dB 게인, Worklet PCM 피크/RMS·클리핑 유지 표시, 기본 OFF 모니터와 독립 음량 구현. 장치·채널 전환·오디오 중단 때 모니터 OFF, 채널 변경 때 미터/클립 초기화. AUTO·음성 보정 옵션 변경·실청취·브라우저 검증은 남음 |
...
| MIDI-01 | 키보드 단축키 | 2 | 부분 구현 | 별도 `feature/phase-2-keyboard-controls` / `40dafae`에 기본 OFF 연주·트랙 선택·녹음/오버더빙/취소/Undo/Redo와 입력 보호 구현. develop과 이번 브랜치에는 미통합, 실제 검증은 남음 |
...
### 2026-09-27 — 입력 채널 선택 / IN-01, 관련 IN-02

- 범위/브랜치: 키보드 기능의 다음 계획에 따라 입력 채널 선택을 진행함. `git fetch origin` 후 최신 develop `e569efc`에서 `feature/phase-1-input-channels`를 생성함. 카운트인 `f7195e0`과 키보드 `40dafae`는 각 브랜치에 보존했고 이번 작업에 병합하지 않음.
- 변경 파일: `src/audio/input/input-channel.ts`, `microphone-input-bus.ts`, `microphone-controller.ts`, `src/components/audio/microphone-channel-select.tsx`, `microphone-input-controls.tsx`, `src/lib/i18n/ko.ts`, `tests/audio/input-channels.test.ts`, `tests/e2e/input-channels.spec.ts`, `README.md`, `docs/plan/README.md`, `docs/plan/NEXT.md`, 이 문서.
- 실제 라우팅: 기본은 기존 mono/speakers 다운믹스. CH1/CH2 선택 시 `ChannelSplitterNode(2)`의 출력 0/1 하나만 입력 게인에 연결함. 이 게인 뒤의 Worklet 입력 하나를 기존 녹음·PCM 미터·모니터 출력이 공유함. Worklet DSP·메시지 계약·공통 시계·PCM/v5 저장 형식은 변경하지 않음. 근거는 [Web Audio 명세의 ChannelSplitterNode](https://www.w3.org/TR/webaudio/#ChannelSplitterNode)와 [다운믹스 규칙](https://www.w3.org/TR/webaudio/#down-mixing). 새 의존성은 없음.
- 지원 범위: 실제 트랙 `getSettings().channelCount`가 양의 정수일 때만 개별 채널을 허용함. 1채널은 CH1, 2채널 이상은 CH1/CH2까지. 채널 수 미보고/유효하지 않음은 합산만 허용함. 스테레오 녹음은 이유와 함께 비활성화하고 다채널에서 개별 선택은 처음 두 채널까지임을 표시함.
- 보호/수명: UI와 컨트롤러에서 준비·대기·녹음·오버더빙 중 채널 변경을 차단함. 입력 연결 재시도도 같은 잠금을 적용해 Worklet의 `input-route`가 캡처를 중단시키지 않도록 함. 오디오 미준비·장치 전환 중 변경을 차단함. 변경 시 모니터를 즉시 OFF로 하고 미터/클립·경로 revision을 초기화함. 같은 채널 재선택과 지원하지 않는 값은 경로를 바꾸지 않음.
- 복구: 새 장치 연결 성공·입력 해제·오디오 전체 재시작 시 기본 합산. 단순 오디오 suspend/resume은 채널을 유지하되 모니터는 OFF. 실패/취소된 장치 전환은 기존 경로·채널을 유지하고 늦게 받은 스트림은 해제함. 라우팅 실패 시 입력을 끊고 오류를 표시하며 재시도는 기본 합산으로 복구함. 선택은 저장하지 않음.
- 작성한 검증: 단위 16개 — 채널 수 누락/잘못된 값 6종, 가용 채널, 경로 중복 방지·해제, 모니터 OFF·미터 revision·클립 초기화, 미지원 CH2 2종, 캡처/중단 잠금, 장치 전환/재시작, 실패/취소/늦은 스트림, 라우팅 오류 재시도, 입력 연결 끊김. E2E 1개 — 제품의 입력 버스를 그대로 번들링하고 합성 스테레오 PCM을 MediaStream으로 전달해 production Worklet의 모노 합산/CH1/CH2 각각 녹음 PCM·입력 피크/RMS·모니터 출력과 1마디 프레임 수를 비교하도록 작성함. **모두 미실행.**
- 정적 검사 중 발견/수정: E2E 변수명 `module`이 Next ESLint 규칙에 걸려 `inputModule`로 수정함. 입력 UI의 추가 조건 분기로 React Doctor가 복잡도 경고 1개·92점을 반환해 연결 상태/재시도 표시를 작은 컴포넌트로 분리함. 오류 무시나 검사 설정 변경은 없음. 최종 검사 결과는 아래에 기록함.
- 최종 실제 검사: `npm run lint` 성공(경고 0개), `npm run typecheck` 성공, `NEXT_TELEMETRY_DISABLED=1 npm run build` 성공(Worklet 37.4kB, `/`·`/_not-found` 정적 빌드). `npx react-doctor@latest --verbose --scope changed` 66개 파일과 전체 `npx react-doctor@latest --verbose` 신규 파일 포함 77개 파일 모두 100/100, 진단 없음. `git diff --check` 공백 오류 없음. 기능 테스트 통과를 의미하지 않음.
- 제한: 사용자 요청에 따라 `npm run test`, `test:audio`, `test:e2e`, 브라우저 조작·실청취를 실행하지 않음. 실제 장치 좌우 분리·무음 채널·채널 수 변경·게인 램프·경로 전환 잡음·UI 키보드/모바일 조작은 미검증. 지원 판정은 연결 시 반환한 채널 수이며 런타임 장치 내부 구성 변화는 별도 감지하지 않음. 스테레오 녹음·AUTO 모니터링·음성 보정 옵션 변경은 남음. IN-01/IN-02와 Phase 1은 부분 구현 상태를 유지함.
- 다음 작업/Git: AUTO 모니터링 범위를 NEXT에 기록함. 관련 파일만 커밋해 작업 브랜치에 push하고 로컬/원격 일치와 깨끗한 작업 트리를 확인한다. develop 통합은 별도 사용자 요청 범위다.

## 알려진 제한과 차단 항목

입력 채널 선택은 모노 합산·CH1/CH2 범위의 구현 완료/미검증 상태다. 스테레오 녹음과 다채널 전체 선택은 제공하지 않으며, 실제 장치의 채널 수가 알려지지 않으면 합산만 허용한다. 연결 중 채널 수가 바뀌는 장치의 동작도 검증이 필요하다. 키보드 연주는 `40dafae`에 별도 구현되어 있으나 이번 브랜치에는 포함되지 않는다.

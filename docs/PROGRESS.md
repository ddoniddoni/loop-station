...
### 2026-09-21 — README 시각 정리 / SYS-01 문서

- 요청: README를 외부 방문자가 보기 좋은 프로젝트 소개로 정리.
- 변경 파일: `README.md`, `docs/assets/readme-hero.svg`, `docs/PROGRESS.md`.
- 변경 내용: 직접 제작한 다크 톤 SVG 배너와 루프 그래픽, 짧은 소개, 기술 스택과 문서 바로가기, 현재/다음 개발 흐름을 배치함. 검사 명령과 폴더 구조는 접기 영역으로 이동하고 실행 예시의 개인 절대 경로를 제거함.
- 상태 표시: Phase 0 진행 중이며 녹음·루핑·저장·클라우드는 미구현임을 유지함. 제품 기능이나 요구사항 완료 상태는 변경하지 않음.
...
| npm run typecheck | next typegen 및 tsc --noEmit 성공 |
| npm run test | 1개 파일, 21개 테스트 통과 |
| npm run build | 성공. / 및 /_not-found 정적 생성 |
| Python 표준 라이브러리 문서 검사 | README 내부 파일 링크, Quick start 앵커, 접기 태그와 코드 펜스, SVG XML과 외부 리소스 미참조 확인 |
| 설치된 sharp로 SVG → PNG 변환 후 이미지 확인 | 1200×440 배너의 텍스트, 루프 그래픽, 여백과 잘림 없음 확인. 검수용 PNG는 /tmp에만 생성 |

- 검증 범위와 제한: 문서와 배너만 변경하여 E2E는 재실행하지 않음. 실제 GitHub 페이지 렌더링과 오디오 하드웨어 검증은 하지 않음.
- 다음 작업: 기존 남은 Phase 0 계획 유지. 사용자 구현 요청 시 AudioContext, Worklet 연결, 마이크 권한과 환경 진단부터 진행.

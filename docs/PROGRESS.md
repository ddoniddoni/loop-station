...
- 다음 작업: 1/2/4/8마디 녹음 길이 선택을 작은 수직 단위로 구현. 테스트 재개 요청 후 다중 트랙 Worklet·마이크·공통 시계·PCM·이력·프로젝트 저장/복구·v1/v2/v3→v4 이전·스테레오 믹서/미터 동작을 검증. 그 전에는 Phase 0/1/2와 관련 요구사항을 검증 완료로 표시하지 않음.
...
사용자가 구현을 요청하면 AGENTS.md와 현재 진행 상태를 읽는다. 다음 기능은 1/2/4/8마디 녹음 길이 선택이다. 모니터링은 기본 OFF로 유지하고 테스트 재개 요청 전에는 자동·브라우저 테스트를 실행하지 않는다. 재개되면 PCM/오버더빙/이력/저장 및 다중 트랙/믹서 단위 테스트와 local-save.spec.ts·mixer.spec.ts·stereo-output.spec.ts부터 실행한다. 실제 Worklet·8트랙 반복/녹음·Undo/Redo·새로고침·v1/v2/v3→v4 이전·팬/마스터/미터 PCM 출력·실패 롤백·다중 탭·Context/장치 수명과 청취 검증이 필요하다. 그 전에는 Phase 0/1/2와 인수 기준을 검증 완료로 표시하지 않는다. 현재 저장 형식은 schemaVersion 4이며 1/2/3 읽기 이전을 지원한다. 다음 변경도 1/2/3/4 데이터를 보존한다.

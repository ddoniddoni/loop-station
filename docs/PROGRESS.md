...
- 프로젝트 위치: `/Users/ddoni/dev/loop-station`, 현재 브랜치 `feature/phase-1-auto-monitoring`. 최신 원격 develop `e569efc`에서 시작함. 이전 파일 백업·Git stash와 다른 기능 브랜치는 보존함.
...
- 별도 구현: 키보드 연주 `feature/phase-2-keyboard-controls` / `40dafae`, 입력 채널 `feature/phase-1-input-channels` / `7321267`도 구현·푸시되어 있으며 develop과 이번 브랜치에는 미통합. 각 브랜치의 단위·E2E도 미실행 상태임.
- 이번 AUTO: OFF/ON/AUTO 선택, Worklet의 실제 캡처 프레임 기반 모니터 출력, 적용 응답·revision·sequence 검사와 장치/오디오 수명에서 OFF 초기화를 구현함. 신규 단위 28개·기존 엔진 구버전 거절 2개·응답 전달 확인·production E2E 2개는 작성만 했으며 미실행.
- 다음 작업: 음성 보정 옵션과 실제 적용 상태. 테스트 재개 요청 후 Phase 0과 AUTO·Tap Tempo·PCM·믹서·저장, 별도 입력 채널·키보드·카운트인의 통합 검증을 진행함.
...
| 1 | 한 트랙 녹음과 공통 시계 | 부분 구현 | 같은 Worklet 시계의 1/2/4/8마디 PCM 캡처·반복·Tap Tempo·AUTO 모니터링 구현. 카운트인·입력 채널은 별도 브랜치에 구현·미통합. 음성 보정 옵션·다른 퀀타이즈 설정과 실제 검증은 남음 |
...
아래 표는 Tap Tempo·녹음 길이 선택·v5 저장·AUTO 모니터링을 포함한 코드 기준이다. 카운트인·키보드·입력 채널은 별도 브랜치에 구현·미통합 상태이며 해당 기록을 함께 확인한다.
...
| IN-01 | 입력 장치와 채널 선택 | 1 | 부분 구현 | 장치 목록·전환과 모노 녹음 라우팅, 캡처 중 장치 변경 차단. 모노 합산·CH1/CH2 선택은 별도 `7321267`에 구현·미통합. 스테레오 녹음과 실제 장치 검증은 남음 |
| IN-02 | 입력 게인과 모니터링 | 1 | 부분 구현 | −24~+24 dB 게인·PCM 미터·클리핑, 기본 OFF의 OFF/ON/AUTO와 독립 음량 구현. AUTO는 실제 캡처 프레임에만 열고 Worklet 적용 응답을 확인함. 장치·입력 해제·오디오 중단 때 OFF. 음성 보정 옵션 변경·실청취·브라우저 검증은 남음 |
...
| MIDI-01 | 키보드 단축키 | 2 | 부분 구현 | 별도 `feature/phase-2-keyboard-controls` / `40dafae`에 구현·푸시. develop과 이번 브랜치에는 미통합, 실제 검증은 남음 |
...
| 로컬 저장 래퍼 | 미선택 |
| Worklet 빌드 도구/메시지 버전 | esbuild 0.28.2, 준비 프로토콜 3. AUTO 모니터의 `input-monitor`/`input-monitor-applied`에 입력 revision·명령 sequence를 사용함. v1 및 별도 카운트인의 v2와 구분함. 트랙 명령·PCM 저장 포맷은 유지 |
...
### 2026-09-27 — AUTO 모니터링 / IN-02

- 범위/브랜치: 직전 입력 채널 브랜치의 NEXT 계획에 따라 진행함. `git fetch origin` 후 최신 develop `e569efc`에서 `feature/phase-1-auto-monitoring`을 생성함. 카운트인 `f7195e0`·키보드 `40dafae`·입력 채널 `7321267`은 각 브랜치에 보존했고 병합하지 않음.
- 변경 파일: `src/audio/input/input-monitor.ts`·`microphone-controller.ts`, `src/audio/loop/pcm-loop.ts`·`pcm-station.ts`, `src/audio/worklets/test-tone-processor.ts`, `src/audio/engine/test-tone-engine.ts`·`worklet-protocol.ts`, `src/components/audio/input-monitor-controls.tsx`·`microphone-input-controls.tsx`, `src/components/studio/studio-input-panel.tsx`, `src/lib/i18n/ko.ts`, `tests/audio/input-monitor.test.ts`·`monitor-controller.test.ts`·`audio-engine.test.ts`, `tests/e2e/auto-monitoring.spec.ts`, README·계획 두 파일·이 문서.
- PCM 계약: 각 트랙의 `nextSample()`은 해당 프레임에 실제 녹음/오버더빙 입력을 썼을 때만 `inputCaptured`를 표시하고 매 프레임 초기화함. 마지막 샘플을 쓴 직후 상태가 playing으로 바뀌어도 이 플래그를 유지함. Station은 모든 트랙의 플래그를 하나로 모아 모니터 출력을 한 번만 열어 중복 출력을 피함. 준비·armed·재생/정지·취소·중단·유효하지 않은 입력에서는 캡처 플래그가 꺼짐. 타이머·React 렌더링·UI 상태 응답은 이 경계를 결정하지 않음.
- 모니터 경로: 기존 Worklet 출력 2에 OFF/ON/AUTO 게이트를 추가함. AUTO는 캡처 프레임의 입력만 출력하고 ON은 트랜스포트와 무관하게 유효 입력을 출력함. 모니터는 ±1 제한과 기존 독립 음량을 사용하며 녹음 PCM·입력 피크/RMS·트랙/마스터 믹서에는 영향을 주지 않음. 트랙의 기존 A 재생과 현재 B 모니터를 분리해 오버더빙의 B가 이중 출력되지 않게 함.
- 모드 적용/수명: 새 ON/AUTO 명령을 보낼 때 물리 출력 게인을 닫고 UI에 적용 대기를 표시함. Worklet이 동일 입력 revision·명령 sequence·mode로 응답했을 때 최신 듣기 음량으로 10ms 램프를 적용함. OFF는 응답을 기다리지 않고 즉시 닫음. 장치 전환 시작·입력 해제·오디오 suspend/재시작 때 OFF로 초기화하고 늦거나 순서가 뒤바뀐 응답은 무시함. 마이크/오디오 자동 시작이나 저장 설정 추가는 없음.
- 프로토콜: 준비 버전 3을 사용함. 이전 v1 Worklet은 모니터 출력에 항상 입력을 내보내므로 새 AUTO UI와 호환되지 않음. 별도 카운트인의 v2와도 구분하여 잘못된 프로세서를 준비 완료로 받지 않도록 함. 저장 형식 v5와 기존 PCM·루프 예약 계약은 유지함. 입력 채널 통합 시 `setMonitorMode`·`monitorMode`/`monitorPending`과 응답을 사용하도록 테스트를 갱신하고, 채널 변경에서 `disableMonitor()` 호출을 유지해야 함. 카운트인 통합 시 준비 버전 3·실제 캡처 플래그를 유지해야 함.
- 작성한 검증: 신규 단위 28개 — 게이트 기본값·모드·오래되거나 잘못된 명령, 8k/44.1k/48kHz·127 BPM·7/8의 첫/마지막 샘플, armed/재생 제외, 오버더빙 A+B와 B 모니터, 취소/정지/입력 중단, 잘못된 샘플, 믹서 Mute·다중 트랙에서 단일 출력, 적용 응답·독립 음량·OFF·중단/해제/재시작·장치 전환 실패·캡처 중 모드 변경. 기존 엔진 단위에는 버전 1/2 거절 사례 2개와 모니터 응답 전달 확인을 추가함. production E2E 2개 — 실제 Worklet의 녹음/오버더빙 모니터 PCM 구간 프레임 수와 원본 PCM/미터 비교, UI OFF/ON/AUTO 선택·적용·마이크 해제/재연결 초기화. **모두 미실행.**
- 실제 검사: `npm run lint` 성공(경고 0개), `npm run typecheck` 성공, `NEXT_TELEMETRY_DISABLED=1 npm run build` 성공(Worklet 39.4kB, `/`·`/_not-found` 정적 빌드). `npx react-doctor@latest --verbose --scope changed` 66개 파일과 전체 `npx react-doctor@latest --verbose` 신규 파일 포함 78개 파일 모두 100/100, 진단 없음. `git diff --check` 공백 오류 없음. 의존성 변경 없음.
- 미실행/제한: 사용자 테스트 중단 요청에 따라 `npm run test`, `test:audio`, `test:e2e`, 브라우저 조작·실청취를 실행하지 않음. AUTO 게이트 경계에서의 클릭/음질과 실제 장치 지연·모바일 조작, 중단/복귀와 다른 기능 브랜치 통합은 미검증. 캡처 구간 밖으로 소리를 늘리지 않는 게이트이므로 경계의 별도 페이드는 없음. 모드 적용 통신/듣기 음량 램프와 물리 출력 지연은 캡처 프레임 경계와 별개임. IN-02 전체·Phase 1은 부분 구현 상태 유지.
- 다음 작업/Git: 음성 보정 옵션과 실제 적용 상태를 NEXT에 기록함. 관련 파일만 커밋해 원격 작업 브랜치에 push하고 로컬/원격 일치와 작업 트리를 확인한다. develop 통합은 별도 요청 범위다.

## 알려진 제한과 차단 항목

AUTO 모니터링의 엔진·모드 적용·초기화는 구현했지만 실제 PCM·브라우저·장치·실청취 검증은 보류 상태다. 입력 채널·키보드·카운트인은 별도 브랜치에 보존되어 있으며 이 AUTO 브랜치에는 포함되지 않는다. 통합 시 모드 API·준비 버전·채널 변경 초기화의 회귀 검증이 필요하다.

...
- 프로젝트 위치: `/Users/ddoni/dev/loop-station`, 현재 브랜치 `feature/phase-1-playback-scheduling`. 최신 원격 develop `e569efc`에서 시작함. 기존 백업·Git stash와 다른 기능 브랜치는 보존함.
...
- 별도 구현: 키보드 `feature/phase-2-keyboard-controls` / `40dafae`, 입력 채널 `feature/phase-1-input-channels` / `7321267`, AUTO `feature/phase-1-auto-monitoring` / `640615e`, 음성 보정 `feature/phase-1-input-processing` / `6c1638b`는 구현·푸시했으나 develop과 이번 브랜치에는 미통합. 각각의 기능 테스트도 미실행임.
- 이번 재생 예약: 트랙별 즉시/다음 박/다음 마디 재생·정지, 현재 상태/예약 분리, 지정 예약 취소·중복 명령·오디오 중단 처리 구현. 단위 30개·구형 준비 응답 거절 3개·production PCM/UI E2E 2개를 작성했으며 모두 미실행. 정적 검사와 빌드는 아래 로그 참조.
- 다음 작업: 녹음 시작 예약 단위 선택. 테스트 재개 요청 후 Phase 0부터 재생 예약·Tap Tempo·PCM·믹서·저장과 별도 기능 브랜치의 통합을 검증함.
...
| 1 | 한 트랙 녹음과 공통 시계 | 부분 구현 | 같은 Worklet 시계의 1/2/4/8마디 PCM·반복·Tap Tempo·재생/정지 시점 선택 구현. 카운트인·채널·AUTO·음성 보정은 별도 브랜치 구현/미통합. 추가 녹음 예약·실제 검증은 남음 |
...
| CLK-03 | 퀀타이즈와 고정 길이 | 1 | 부분 구현 | 다음 마디 녹음·1/2/4/8마디 첫 반복, 완성된 루프의 즉시/다음 박/다음 마디 재생·정지와 예약 취소 구현. 예약은 실제 블록 두 개의 여유와 절대 tick/frame 기준. 녹음 시작 선택·추가 그리드·카운트인 통합·실제 경계 검증은 남음 |
...
### 2026-09-27 — 반복 재생·정지 시점 선택 / CLK-03

- 범위/기준: 직전 음성 보정 브랜치 NEXT의 다음 기능과 상세 명세 5.4~5.5절. `git fetch origin develop` 후 `e569efc`에서 `feature/phase-1-playback-scheduling`을 생성했다. 카운트인·키보드·채널·AUTO·음성 보정은 별도 브랜치에 보존했으며 이번 변경에 병합하지 않았다.
- 변경 파일: `src/audio/loop/{playback-scheduling,pcm-loop,loop-protocol,loop-controller}.ts`, `src/audio/engine/worklet-protocol.ts`, `src/components/studio/{playback-controls,recording-track}.tsx`, `src/components/studio/track-availability.ts`, `src/lib/i18n/ko.ts`, `tests/audio/{playback-scheduling,playback-controller,audio-engine,loop-controller,station-controller}.test.ts`, `tests/e2e/playback-scheduling.spec.ts`, `README.md`, `docs/plan/{README,NEXT}.md`, 이 기록.
- 실행 계약: 재생은 기본 다음 마디, 정지는 기본 즉시다. 각 트랙에 즉시/다음 박/다음 마디를 선택할 수 있다. 박은 현재 박자표 분모 기준이며 BPM은 계속 4분음표 기준이다. 오디오 스레드에서 명령 도착 시 최근 실제 블록 길이 두 개의 여유를 가진 다음 절대 tick/frame 경계를 고른다. 즉시는 다음 처리 블록에 반영한다. 트랙당 예약 하나만 보관하고 블록 중간에서도 해당 프레임 전에 상태를 전환하여 시작 프레임 포함·정지 프레임 제외를 구현했다. React 타이머와 새로운 per-sample 배열 생성은 없다.
- 상태/충돌: `pendingPlayback`의 동작·시점·sequence·frame·tick을 실제 phase와 별도로 보고한다. 재생 예약은 stopped, 정지 예약은 playing을 유지한다. 메인 스레드는 전송 대기만 먼저 표시하고 실제 phase/예약은 일치하는 sequence의 응답으로 갱신한다. 중복 전송과 대기 중 시점 변경·오버더빙·이력·비우기를 막는다. 지정한 예약 ID만 취소하며 이미 실행된 명령은 되돌리지 않는다. 원래 PCM·Undo/Redo·저장본은 변경하지 않는다.
- 수명/기본값: 상단 전체 정지·위치 초기화·오디오 중단은 즉시 정지와 예약 제거를 유지한다. 재개/재연결은 자동 재생하지 않는다. 마이크 해제만으로 확정된 루프의 재생 예약은 취소하지 않는다. 개별 트랙 선택은 탭에서만 유지하며 저장 스키마는 v5 그대로다. 상단 전체 재생은 기존 다음 마디 정책을 유지한다. 기존 녹음/오버더빙 시작·길이·종료 정책은 이번 범위 밖이다.
- UI: 완성된 트랙에 재생/정지 시점 선택과 잠금 이유를 추가했다. 현재 PLAYING/STOPPED 표시와 예약 안내를 구분하고 주 버튼으로 해당 예약을 취소한다. 박 단위·가까운 경계 건너뜀·물리 지연과 전체 조작의 차이를 안내한다. 부분 녹음은 이유와 함께 선택을 비활성화한다. 마이크 권한을 자동 요청하지 않는다.
- 프로토콜: 준비 버전 4로 변경하고 이전 1·2·3을 거절한다. loop-status의 pendingPlayback과 phase/pendingPlay/sequence 일관성을 검사한다. 구형 테스트 상태 생성 2곳을 새 계약에 맞췄다. 입력 채널/키보드/음성 보정과 통합 시 테스트 상태도 갱신하고, AUTO의 실제 캡처 플래그와 카운트인 예약·v5 길이 계약을 보존해야 한다. 통합 시 버전은 4 이상을 유지한다.
- 작성한 검증: DSP/프로토콜 단위 24개 — 3/4·4/4·6/8·7/8 단위, 8/44.1/48kHz, 소수 프레임과 가변 블록, 두 블록 여유·늦은 도착, 재생/정지 첫·마지막 PCM, 취소·중복·오래된 ID·늦은 취소, 전체 정지·마이크 해제·복구·편집 충돌, 1/8마디 혼합 트랙. 컨트롤러 단위 6개 — 설정/PCM/권한 분리, 전송과 실제 상태 구분, 늦은 응답·취소, 정지 예약 잠금, suspend와 새 엔진 복구. 기존 엔진 테스트에 구형 버전 거절 3개 추가. production E2E 2개 — 실제 Worklet 합성 PCM의 재생~정지 프레임 수와 UI의 정지 예약/취소/즉시 정지. **모두 미실행이다.**
- 실제 검사: 최종 `npm run lint`·`npm run typecheck` 모두 종료 코드 0. `NEXT_TELEMETRY_DISABLED=1 npm run build` 종료 코드 0, Worklet 39.8kB와 Next.js 16.3.5 webpack의 `/`·`/_not-found` 정적 페이지 생성 확인. `npx react-doctor@latest --verbose --scope changed` 66파일·100/100, 신규 파일 포함 `npx react-doctor@latest --verbose` 전체 78파일·100/100, 진단 없음. `git diff --check` 공백 오류 없음. 중간 타입 검사의 명령 객체 매개변수 타입 불일치와 전체 React Doctor의 일반 함수 export 경고를 수정한 뒤 다시 확인했다. 테스트 삭제·검증 무시·의존성 변경은 없다.
- 미실행/제한: 사용자 중단 요청대로 `npm run test`, `test:audio`, `test:e2e`, 브라우저 조작·실청취 미실행. 예약/오디오 중단·실제 PCM·모바일 배치/접근성·물리 장치 지연·정지 경계 클릭과 다른 기능 통합은 미검증. 별도 경계 페이드, 1/16·1/8 등 음표 그리드 선택, 여러 마디/현재 루프 경계와 녹음 시작 시점 선택은 후속 범위다. CLK-03 전체와 Phase 0/1/2는 부분 구현 상태를 유지한다.
- 다음 작업/Git: 녹음 시작 예약 단위 선택을 NEXT에 기록했다. 허용된 검사 후 관련 파일만 커밋·작업 브랜치에 push하고 로컬/원격 일치와 작업 트리를 확인한다. develop 통합은 별도 요청 범위다.

## 알려진 제한과 차단 항목

재생·정지 시점 선택과 예약 취소는 구현했으나 실제 PCM·브라우저·실청취 검증은 보류 상태다. 카운트인·키보드·채널·AUTO·음성 보정은 별도 브랜치 구현/미통합 상태다.

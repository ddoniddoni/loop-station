...
- 프로젝트 위치: `/Users/ddoni/dev/loop-station`, 현재 브랜치 `feature/phase-1-input-processing`. 최신 원격 develop `e569efc`에서 시작함. 이전 백업·Git stash와 다른 기능 브랜치는 보존함.
...
- 별도 구현: 키보드 `feature/phase-2-keyboard-controls` / `40dafae`, 입력 채널 `feature/phase-1-input-channels` / `7321267`, AUTO 모니터링 `feature/phase-1-auto-monitoring` / `640615e`는 구현·푸시했으나 develop과 이번 브랜치에는 미통합. 각 브랜치의 테스트도 미실행임.
- 이번 음성 보정: 세 옵션의 같은 트랙 적용·장치 지원 사유·요청/보고값·실패/중단·녹음 잠금·모니터 OFF를 구현함. 단위 24개·production UI E2E 2개를 작성했으며 모두 미실행. 정적 검사와 빌드 결과는 아래 로그 참조.
- 다음 작업: 반복 재생·정지의 실행 시점 선택. 테스트 재개 요청 후 Phase 0부터 음성 보정·Tap Tempo·PCM·믹서·저장과 별도 기능 브랜치의 통합 검증을 진행함.
...
| 1 | 한 트랙 녹음과 공통 시계 | 부분 구현 | 같은 Worklet 시계의 1/2/4/8마디 PCM·반복·Tap Tempo·음성 보정 옵션 구현. 카운트인·채널 선택·AUTO는 별도 브랜치 구현/미통합. 다른 퀀타이즈 설정·실제 검증은 남음 |
...
| IN-02 | 입력 게인과 모니터링 | 1 | 부분 구현 | 모노 입력 버스, −24~+24 dB 게인, Worklet PCM 미터, 기본 OFF 모니터와 독립 음량 구현. 음성 보정 선택·장치 지원·요청/보고값 구분·잠금·오류 처리 구현. AUTO는 별도 브랜치 구현/미통합. 실제 장치·브라우저·실청취 검증은 남음 |
...
### 2026-09-27 — 음성 보정 옵션과 브라우저 보고값 / IN-02

- 범위/기준: 상세 명세 5.3절과 직전 AUTO 브랜치 NEXT의 다음 작업. 최신 원격 develop `e569efc`에서 `feature/phase-1-input-processing`을 생성했다. 카운트인·키보드·채널·AUTO 코드는 각 브랜치에 보존했고 이번 변경에 병합하지 않았다. 현재 모니터는 기본 develop의 OFF/ON, Worklet 준비 프로토콜은 1이다.
- 변경 파일: `src/audio/input/{input-processing,microphone-session,microphone-controller}.ts`, `src/components/audio/{input-processing-controls,microphone-input-controls,microphone-setup}.tsx`, `src/lib/i18n/ko.ts`, `tests/audio/input-processing.test.ts`, `tests/e2e/input-processing.spec.ts`, `README.md`, `docs/plan/{README,NEXT}.md`, 이 기록.
- API 결정: 새 스트림을 요청하지 않고 기존 트랙의 `applyConstraints()`를 사용한다. 선택 옵션은 exact Boolean이고 `getConstraints()`의 다른 옵션·장치 제약을 유지한다. 브라우저 전역 지원과 트랙의 양쪽 Boolean capability를 확인한 옵션만 변경하며 고정 ON/OFF·미지원·확인 불가를 구분한다. capability는 설정 조합 성공을 보장하지 않으므로 거절도 처리한다.
- 상태와 수명: 적용 시작 시 모니터를 즉시 OFF로 하고 `applying`으로 새 녹음·장치/입력 변경을 잠근다. 입력 revision을 갱신해 적용 중 라우트를 비활성으로 알리고 완료 뒤 현재 오디오 상태에 맞춰 복구한다. 기존 스트림·연결 노드·입력 게인은 유지한다. 실패 시 기존 요청값을 유지하고 보고값을 다시 읽으며 실패한 옵션·선택값을 별도로 안내한다. 해제·장치 종료·dispose·재연결의 늦은 응답은 폐기한다. ended 트랙의 fulfilled Promise도 성공으로 다루지 않는다.
- UI: 음악 입력 기본 OFF 요청, 세 옵션 선택, 요청값과 브라우저 보고값, 미지원/고정/불명 사유, 적용 대기·거절·불일치를 표시한다. 읽기 실패/누락은 확인 불가이며 오래된 값으로 성공을 꾸미지 않는다. 불일치 시 같은 요청값 재적용과 수동 새로 확인을 제공한다. 보고값은 연결·적용 완료·수동 확인 시점 기준이며 음질 측정이 아니다. 설정은 저장하지 않고 새 장치는 OFF 요청, 전환 실패는 이전 요청값을 유지한다.
- 작성한 검증: 단위 24개 — 지원/고정/불명과 보고값 판별, 명시적 권한 요청, 같은 스트림·다른 제약 보존, 즉시 모니터 OFF, 중복 적용·장치 변경 차단, 거절과 재시도, 실제 보고 불일치/누락, 녹음과 적용의 상호 잠금, 해제/끊김/dispose/무통지 ended/오디오 중단/재연결의 늦은 응답. E2E 2개 — production UI·Worklet·합성 Chromium 마이크에서 보정 보고와 거절/대기를 주입해 비활성 사유·요청/보고값·재적용·실패·해제 후 초기화를 확인하는 시나리오. **모두 미실행이며 실제 처리 효과를 검증하지 않았다.**
- 실제 검사: 최종 `npm run lint`·`npm run typecheck` 모두 종료 코드 0. `NEXT_TELEMETRY_DISABLED=1 npm run build` 종료 코드 0, Worklet 37.4kB 생성과 Next.js 16.3.5 webpack 정적 페이지 빌드 확인. `npx react-doctor@latest --verbose --scope changed` 66파일·100/100, 신규 파일을 포함한 `npx react-doctor@latest --verbose` 전체 77파일·100/100으로 경고 없음. `git diff --check` 공백 오류 없음. 중간 타입 검사의 테스트 정리 API 오기(`dispose` 대신 실제 `detach`)와 React Doctor의 입력 컨트롤 조건 복잡도 경고를 수정한 뒤 최종 검사를 통과했다. 경고 억제나 의존성 변경은 없다.
- 남은 검증: 사용자 요청에 따라 단위·오디오·E2E·브라우저 조작·실청취 미실행. 실제 브라우저·장치의 applyConstraints/getSettings/설정 조합, 보정 음질·입력 지연·스테레오 채널 변화와 모바일 접근성은 미검증. 응답 없는 적용은 마이크를 해제해야 벗어나며 타임아웃으로 적용 취소를 가장하지 않는다. 브라우저가 범위를 보고하지 않으면 옵션을 비활성화하는 보수적 정책이다.
- 통합 주의: AUTO의 새 모드/응답 API에 테스트를 맞추고 적용 시작 `disableMonitor()`와 적용 중 라우트 비활성 조건을 유지한다. 채널 선택 통합 시 보정 후 바뀔 수 있는 채널 수에 대해 기존 선택을 재검증한다. 카운트인·키보드도 적용 중에는 캡처를 시작하지 않아야 한다. 저장 스키마/PCM/Worklet DSP 변경은 없다. IN-02 전체와 Phase 0/1/2는 부분 구현 상태다.
- 근거: [W3C Media Capture — applyConstraints](https://www.w3.org/TR/mediacapture-streams/#dom-constrainablepattern-applyconstraints), [getCapabilities/getSettings와 트랙 수명](https://www.w3.org/TR/mediacapture-streams/#dom-mediastreamtrack-applyconstraints). 2026-09-27 확인. 거절 시 기존 제약 유지, settings와 물리 측정의 차이, ended 트랙의 Promise 처리와 capabilities 조합 한계를 반영했다.
- 다음 작업/Git: 반복 재생·정지의 실행 시점 선택을 NEXT에 기록했다. 허용된 검사 후 관련 파일만 커밋·원격 작업 브랜치에 push하고 상태를 확인한다. develop 통합은 별도 요청 범위다.

## 알려진 제한과 차단 항목

음성 보정 옵션은 구현했으나 브라우저·실제 장치 처리와 UI 검증은 보류 상태다. 키보드·채널 선택·AUTO·카운트인은 각 브랜치에 구현·미통합 상태이며 해당 기록과 함께 확인한다.
...
시작 시 `AGENTS.md`, [개발 로드맵](plan/README.md), [현재 작업 계획](plan/NEXT.md), 이 기록을 읽는다. 다음 기능은 반복 재생·정지의 실행 시점 선택이다. 음성 보정·Tap Tempo·1/2/4/8마디 녹음 길이는 구현 완료/미검증이며 카운트인·키보드·채널·AUTO는 각 브랜치의 구현·미통합 상태다. 테스트 재개 요청 후 Phase 0 수명·권한·장치 해제부터 음성 보정·PCM·믹서·v1~v4→v5 저장 이전·10분 혼합 길이 동기를 확인한다. 그 전에는 Phase 0/1/2와 인수 기준을 검증 완료로 표시하지 않는다. 현재 schemaVersion 5와 기존 데이터·선택 길이 고정·PCM ticks 기준을 유지한다. 후속 범위와 통합 주의는 NEXT.md를 따른다.

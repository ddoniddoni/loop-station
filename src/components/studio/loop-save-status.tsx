...
export function LoopSaveStatus() {
  ...
  return <Popover.Root>
    <Popover.Trigger>
      <Button className="station-save-trigger" variant="ghost" color={problem ? "amber" : "gray"} aria-label={`로컬 저장 상태: ${saveStatusLabel(save)}`}>
        <span role="status">{saveStatusLabel(save)}</span><span aria-hidden="true"> · LOCAL</span>
      </Button>
    </Popover.Trigger>
    <Popover.Content className="station-overlay" width="320" sideOffset={8}>
      <Heading as="h2" size="3">로컬 저장</Heading>
      <Text as="p" size="2" mt="2">8개 트랙의 녹음·오버더빙과 Undo/Redo·비우기 복구 이력을 하나의 프로젝트로 이 브라우저에 자동 저장합니다.</Text>
      <Text as="p" size="2" mt="2" role="status">{saveStatusLabel(save)}{save.savedAtLabel ? ` · 마지막 저장 ${save.savedAtLabel}` : ""}</Text>

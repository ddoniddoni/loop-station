...
export type SavedStationSession = {
  schemaVersion: 4; revision: string; updatedAt: number; history: StationProject; checksum: string;
};
...
async function checksum(project: StationProject): Promise<string> {
  ...
  return digest(new TextEncoder().encode(JSON.stringify({ schemaVersion: 4, tracks: hashes, mixer, master: { gainDb: project.master.gainDb, mute: project.master.mute } })).buffer);
}
export async function createStationSession(history: StationProject): Promise<SavedStationSession> {
  ...
  return { schemaVersion: 4, revision: crypto.randomUUID(), updatedAt: Date.now(), history, checksum: await checksum(history) };
}
export async function decodeStationSession(value: unknown): Promise<SavedStationSession> {
  ...
  if (value.schemaVersion === 1) {
    ...
    return { schemaVersion: 4, revision: legacy.revision, updatedAt: legacy.updatedAt, history, checksum: await checksum(history) };
  }
  if (typeof value.schemaVersion === "number" && value.schemaVersion > 4) throw new LoopStorageError("version", "더 새로운 앱에서 저장한 프로젝트입니다. 저장본은 변경하지 않았습니다.");
  if ((value.schemaVersion !== 2 && value.schemaVersion !== 3 && value.schemaVersion !== 4) || !("history" in value)
  ...
  } else if (value.schemaVersion === 4 && validProject(value.history)) {
    history = value.history;
    actualChecksum = await checksum(history);
  } else {
  ...
  history.tracks.forEach(assertFiniteSamples);
  return { schemaVersion: 4, revision: value.revision, updatedAt: value.updatedAt, history,
    checksum: value.schemaVersion !== 4 ? await checksum(history) : actualChecksum };
}
